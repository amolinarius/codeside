    ...
    return currentCourse ? <CoursePage course={currentCourse} setCourse={setCurrentCourse}/> : (
        <div className="px-5">
            {sections.map(section => (<React.Fragment key={"section"+section.id}>
                <h1 className="text-2xl font-bold mb-4">{toRomanNumerals(section.id+1)} - {section.title}</h1>
                {section.courses.map(course => (
                    <div key={"course"+course.id} className="relative ml-2 px-5 bg-base-200 py-4">
                        <h2 className="text-lg font-semibold">{course.id+1}. {course.title}</h2>
